...
public class DocumentPipelineOutboxServiceImpl
        extends ServiceImpl<DocumentPipelineOutboxMapper, DocumentPipelineOutbox>
    ...
    @Override
    public List<DocumentPipelineOutbox> claimPublishableMessages(String lockOwner, LocalDateTime now) {
        ...
        List<DocumentPipelineOutbox> candidates = outboxMapper.selectPublishable(
                now, expiredLockTime, properties.getBatchSize());

        // 2. 逐条使用原状态和原锁时间竞争发布权
        List<DocumentPipelineOutbox> claimed = new ArrayList<>();
        for (DocumentPipelineOutbox candidate : candidates) {
    ...
    public void markPublishFailed(Long outboxId, String failureReason) {
        DocumentPipelineOutbox outbox = outboxMapper.selectById(outboxId);
        if (outbox == null) {
