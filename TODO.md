...
- [ ] 预留文档流水线可靠消息发布模式配置，支持 Outbox 与 RocketMQ 事务消息切换，首期默认使用 Outbox。
- [ ] 为文档流水线实现 Redis Stream 消息适配器，与现有 RocketMQ 实现共用消息处理契约。
- [ ] 实现文档任务最终失败告警的飞书与邮件适配，并通过独立告警 Outbox 可靠投递。
- [ ] 实现告警渠道前端管理页：仅管理员可配置启停、收件人和脱敏后的目标信息；飞书 Webhook、SMTP 密码等敏感值由后端加密保存且永不下发前端。
- [ ] 支持按告警严重级别配置飞书 Webhook 与邮件收件人，实现 `WARNING`、`ERROR` 等通知目标分流。
- [ ] 多租户权限模型。
- [ ] 文档版本管理。
...
- [ ] 将辅助任务 Prompt 拆分为“系统指令模板 + 用户输入模板”，通过数据库 Prompt 版本迁移升级问题改写、意图识别和标题生成的角色结构，并兼容历史已发布 Prompt。
- [ ] 评估 Embedding 主备的向量空间一致性约束：跨模型切换时引入索引版本、全量重建与原子切换，避免不同语义空间的向量混用；当前阶段已完成 `ModelGatewayEmbeddingModel` 与 Spring AI `MilvusVectorStore` 的桥接切换。
- [ ] 补齐 RAG 检索引用的 SSE 事件与历史引用数据契约，并同步 Apifox 接口文档。
- [ ] 知识库前端提供完整高级处理配置表单，包括 OCR、图片描述、切分策略与索引开关。
- [ ] 知识库文档详情支持在新标签查看原文件和解析结果，并在接入前明确对象存储地址的访问权限策略。
- [ ] 后端文档列表接口支持按处理状态筛选，并在知识库前端增加“处理中”“处理失败”等服务端状态视图。
